...
def grab_image_in(left=0, top=0, right=100, bottom=100, dev=False,
    ...
    try:
        ...
        if width <= 0 or height <= 0:
            raise ValueError(
                "Invalid region dimensions. Width and height must be positive.")
...
def detect_red(image, dev=False) -> bool:
    ...
    # Lower bound for red (second range)
    lower_red2 = np.array([170, 120, 70])
    # Upper bound for red (second range)
...
def img_is_visible(image_path, left=0, top=0, right=100,
    ...
    try:
        gui.locateOnScreen(image_path, confidence=confidence,
                           region=(left, top, width, height))
        return True
    except:
        if verbose:
            print(
                f"Cannot find {image_path} on screen in region ({left}, {top}, {right}, {bottom}) with confidence {confidence}.")
        return False
...
def img_is_visible_grayscale(image_path, left=0, top=0, right=100, bottom=100,
    ...
    if len(loc[0]) > 0:
        ...
        if verbose:
            print(
                f"Cannot find {image_path} in region ({left},{top},{right},{bottom}) with confidence {confidence}")
        return False


def get_img_coordinate(image_path, left=0, top=0, right=100,
                       bottom=100, confidence=0.7,
                       silence=True) -> Optional[Tuple[int, int]]:
    """
    ...
    try:
        img_location = gui.locateOnScreen(
            image_path, confidence=confidence, region=(left, top, width, height))
...
def find_all_image(match_image, left=0, top=0, right=100,
    ...
    if dev:
        for pt in zip(*loc[::-1]):
            cv2.rectangle(
                image, pt, (pt[0] + template.shape[1], pt[1] + template.shape[0]), (0, 255, 0), 2)
        plt.imshow(image)
        plt.show()
    ...
    for pt in zip(*loc[::-1]):
        ...
        rectangles.append(
            [pt[0], pt[1], pt[0] + template.shape[1], pt[1] + template.shape[0]])
    ...
    for (l, t, r, b) in rectangles:
        # middle_x + the original abosulte x
        center_x = l + (r - l) // 2 + offset_x
        # middle_y + the original abosulte y
        center_y = t + (b - t) // 2 + offset_y
        centers.append((center_x, center_y))

    return centers


def click_with_image(image_path, left=0, top=0, right=100, bottom=100,
                     confidence=0.7, duration=0.2, silence=False) -> bool:
    """
    Click on the image
    Args:
        image_path (str): path to the image
    ...
    """
    coordinates = get_img_coordinate(
        image_path, left, top, right, bottom, confidence, silence)
