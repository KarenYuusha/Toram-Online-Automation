...
def filtered_bag(selected_type='collectible', dev=False) -> None:
    ...
    if selected_type not in bag_configs:
        raise ValueError(
            f"Invalid bag type: {selected_type}. Valid options are: {list(bag_configs.keys())}")
    ...
    def toggle_bag(left, top, right, bottom, click_x, click_y, flag=True) -> None:
        ...
        should_click = (flag and not is_blue_detected) or (
            not flag and is_blue_detected)
        if should_click:
...
def click_grid_pos(row, col, base_x=INITIAL_X, base_y=INITIAL_Y, delay=0.1) -> None:
    x = base_x + col * COL_WIDTH
    y = base_y + row * ROW_HEIGHT
    click_relative(x, y, duration=delay)
...
def proc_eq(item_limit=35, style=False, hard_code=True, dev=False) -> bool:
    ...
    progress_bar = tqdm(
        total=item_limit, desc="Processing Items") if style else None
    ...
    for scroll in range(scroll_limit):
        ...
        if hard_code:
            ...
            # compute how many full rows are left
            remaining_rows = (remaining_items + 4) // 5
            ...
            for row in range(4):
                ...
                for col in range(5):
                    ...
                    if style:
                        # Update progress bar for each click
                        progress_bar.update(1)

        # use matching template
        else:
            res = find_all_image(
                'asset/images/smith/armor.png', 49, 16, 97, 90, dev=dev)
            for _ in res:
